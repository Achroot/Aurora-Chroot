...
chroot_lock_is_stale() {
  ...
  if [[ ! -f "$owner_file" ]]; then
    # Another process may have just created the lockdir and not yet written owner.pid.
    # Treat as in-progress until it is clearly old.
    age="$(chroot_lock_age_sec "$lock_path" 2>/dev/null || true)"
  ...
  if kill -0 "$owner_pid" >/dev/null 2>&1; then
    # When lock metadata includes proc starttime, verify PID identity to detect reuse.
    # If metadata is missing or unreadable, prefer treating live owner PID as active.
...
chroot_lock_release() {
  ...
  if [[ "$owner_pid" =~ ^[0-9]+$ && "$owner_pid" != "$$" ]]; then
    ...
    # Only clean up when the lock is verifiably stale.
    chroot_lock_is_stale "$lock_path" || return 0
  elif [[ ! "$owner_pid" =~ ^[0-9]+$ ]]; then
    # Corrupted/missing owner metadata: only remove clearly stale lockdirs.
    chroot_lock_is_stale "$lock_path" || return 0
  fi

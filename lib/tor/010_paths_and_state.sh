...
chroot_tor_performance_file() {
  local distro="$1"
  printf '%s/performance.json' "$(chroot_tor_state_dir "$distro")"
}

chroot_tor_performance_request_file() {
  local distro="$1"
  printf '%s/performance.request' "$(chroot_tor_state_dir "$distro")"
}
...
chroot_tor_probe_rootfs_config_dir() {
  local distro="$1"
  printf '%s/performance-probe' "$(chroot_tor_rootfs_config_dir "$distro")"
}

chroot_tor_probe_rootfs_torrc_file() {
  local distro="$1"
  printf '%s/torrc' "$(chroot_tor_probe_rootfs_config_dir "$distro")"
}
...
chroot_tor_probe_rootfs_runtime_dir() {
  local distro="$1"
  printf '%s/performance-probe' "$(chroot_tor_rootfs_runtime_dir "$distro")"
}
...
chroot_tor_probe_rootfs_data_dir() {
  local distro="$1"
  printf '%s/data' "$(chroot_tor_probe_rootfs_runtime_dir "$distro")"
}
...
chroot_tor_probe_rootfs_pid_file() {
  local distro="$1"
  printf '%s/tor.pid' "$(chroot_tor_probe_rootfs_runtime_dir "$distro")"
}
...
chroot_tor_probe_rootfs_control_cookie_file() {
  local distro="$1"
  printf '%s/control_auth_cookie' "$(chroot_tor_probe_rootfs_runtime_dir "$distro")"
}
...
chroot_tor_default_performance_json() {
  cat <<'JSON'
{
  "active": false,
  "controller_pid": null,
  "controller_pid_starttime": null,
  "current_exit_country_code": "",
  "current_exit_fingerprint": "",
  "current_exit_ip": "",
  "current_exit_nickname": "",
  "current_score": {
    "latency_ms": null,
    "speed_mbps": null
  },
...
chroot_tor_performance_state_json() {
  local distro="$1"
  local perf_file
...
chroot_tor_performance_clear() {
  local distro="$1"
  local perf_file req_file tmp
  perf_file="$(chroot_tor_performance_file "$distro")"
  req_file="$(chroot_tor_performance_request_file "$distro")"
  tmp="$perf_file.$$.tmp"
  chroot_tor_ensure_state_layout "$distro"
  chroot_tor_default_performance_json >"$tmp"
  mv -f -- "$tmp" "$perf_file"
  rm -f -- "$req_file"
}
...
chroot_tor_performance_write_json() {
  local distro="$1"
  local json_text="$2"
  local perf_file tmp
  perf_file="$(chroot_tor_performance_file "$distro")"
...
try:
    ...
    raise SystemExit(f"invalid performance json: {exc}")
if not isinstance(data, dict):
    raise SystemExit("invalid performance json: payload must be an object")
with open(out_path, "w", encoding="utf-8") as fh:
    json.dump(data, fh, indent=2, sort_keys=True)
    fh.write("\n")
PY
...
chroot_tor_performance_merge_json() {
  local distro="$1"
  local patch_json="$2"
  local perf_file tmp
  perf_file="$(chroot_tor_performance_file "$distro")"
  ...
  chroot_tor_ensure_state_layout "$distro"
  chroot_require_python
  "$CHROOT_PYTHON_BIN" - "$perf_file" "$tmp" "$patch_json" <<'PY'
import json
import os
import sys

src_path, out_path, patch_text = sys.argv[1:4]
...
try:
    ...
    raise SystemExit(f"invalid performance patch json: {exc}")
if not isinstance(patch, dict):
    raise SystemExit("invalid performance patch json: payload must be an object")
...
chroot_tor_performance_request() {
  local distro="$1"
  local reason="${2:-reselect}"
  local req_file tmp
  req_file="$(chroot_tor_performance_request_file "$distro")"
  tmp="$req_file.$$.tmp"
  chroot_tor_ensure_state_layout "$distro"
...
chroot_tor_performance_clear_request() {
  local distro="$1"
  rm -f -- "$(chroot_tor_performance_request_file "$distro")"
}

chroot_tor_performance_request_reason() {
  local distro="$1"
  local req_file
  req_file="$(chroot_tor_performance_request_file "$distro")"
  [[ -f "$req_file" ]] || return 1
...
chroot_tor_performance_controller_pid() {
  local distro="$1"
  local perf_file
  perf_file="$(chroot_tor_performance_file "$distro")"
...
chroot_tor_performance_controller_identity_tsv() {
  local distro="$1"
  local perf_file
  perf_file="$(chroot_tor_performance_file "$distro")"
  [[ -f "$perf_file" ]] || {
...
start = data.get("controller_pid_starttime")
if isinstance(start, int):
    start_text = str(start)
elif isinstance(start, str) and start.strip().isdigit():
    start_text = start.strip()
else:
    start_text = ""

print(f"{pid_text}|{start_text}")
PY
}

...
chroot_tor_apps_refresh() {
  ...
  if ! chroot_tor_root_copy_file /data/system/packages.list "$packages_list_file"; then
    ...
    if [[ "$fatal" == "0" ]]; then
      chroot_log_warn tor "apps-refresh failed distro=$distro source=/data/system/packages.list"
      return 1
    fi
...
PY
  ...
  chroot_log_info tor "apps-refresh distro=$distro source=$uid_source"
}
...
chroot_tor_apps_ensure() {
  ...
  if [[ ! -f "$apps_file" ]]; then
    chroot_log_run_internal_command tor tor.apps.refresh "$distro" "$distro" tor apps refresh --json -- chroot_tor_apps_refresh "$distro"
  fi
}

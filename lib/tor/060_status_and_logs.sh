...
chroot_tor_status_json() {
  ...
  local daemon_pid daemon_start rules_active=0 bootstrap_complete=0
  ...
  local bypass_count exit_count exit_strict exit_performance exit_performance_ignore_count
  local saved_exit_codes saved_exit_resolved saved_perf_ignore_codes saved_perf_ignore_resolved
  ...
  IFS=$'\t' read -r bypass_count exit_count exit_strict exit_performance exit_performance_ignore_count <<<"$(chroot_tor_config_summary_tsv "$distro")"
  IFS=$'\t' read -r saved_exit_strict saved_exit_codes saved_exit_resolved <<<"$(chroot_tor_exit_resolved_tsv "$distro")"
  IFS=$'\t' read -r saved_perf_ignore_codes saved_perf_ignore_resolved <<<"$(chroot_tor_exit_performance_ignore_resolved_tsv "$distro")"
  ...
  if [[ "${CHROOT_TOR_STATUS_APPS_REFRESH_OK:-}" == "1" ]]; then
    apps_refresh_ok=1
  elif [[ "${CHROOT_TOR_STATUS_APPS_REFRESH_OK:-}" == "0" ]]; then
    apps_refresh_ok=0
  elif chroot_tor_apps_refresh "$distro" 0 >/dev/null 2>&1; then
    apps_refresh_ok=1
  fi
  ...
  if [[ -n "$daemon_pid" ]] && chroot_tor_bootstrap_complete "$distro"; then
    bootstrap_complete=1
  fi
  ...
  chroot_require_python
  "$CHROOT_PYTHON_BIN" - "$status_file" "$targets_file" "$apps_file" "$distro" "$enabled_saved" "$identity_mode_saved" "$daemon_user_saved" "$uid_saved" "$gid_saved" "$termux_saved" "$activated_at_saved" "$last_error_saved" "$daemon_pid" "$daemon_start" "$rules_active" "$bootstrap_complete" "$nat_jump" "$filter_jump" "$filter6_jump" "$policy_v4_active" "$policy_v6_active" "$tor_bin_host" "$install_backend" "${CHROOT_TOR_IPTABLES_BIN:-}" "${CHROOT_TOR_IP6TABLES_BIN:-}" "${CHROOT_TOR_IP_BIN:-}" "${CHROOT_TOR_UID_SOURCE:-}" "$(chroot_tor_state_dir "$distro")" "$(chroot_tor_rootfs_torrc_file "$distro")" "$(chroot_tor_rootfs_pid_file "$distro")" "$(chroot_tor_targets_file "$distro")" "$CHROOT_TOR_DEFAULT_SOCKS_PORT" "$CHROOT_TOR_DEFAULT_TRANS_PORT" "$CHROOT_TOR_DEFAULT_DNS_PORT" "$active_distro" "$active_activated_at" "$family" "$bypass_count" "$exit_count" "$exit_strict" "$exit_performance" "$exit_performance_ignore_count" "$saved_exit_codes" "$saved_exit_resolved" "$saved_perf_ignore_codes" "$saved_perf_ignore_resolved" "$apps_refresh_ok" "$public_exit_json" "$active_exit_json" <<'PY'
import json
import os
import sys
...
(
    ...
    exit_count_text,
    exit_strict_text,
    exit_performance_text,
    exit_performance_ignore_count_text,
    saved_exit_codes,
    saved_exit_resolved,
    saved_perf_ignore_codes,
    saved_perf_ignore_resolved,
...
performance_doc = parse_json_file(os.path.join(state_dir, "performance.json"), {"active": False})
if not isinstance(performance_doc, dict):
    performance_doc = {"active": False}
performance_doc.setdefault("active", False)
performance_doc["saved_enabled"] = exit_performance_text == "1"
...
run_mode_saved = str(status_doc.get("run_mode", "") or "")
...
performance_expected = bool(enabled and run_mode_saved in {"configured", "configured-exit"} and performance_doc.get("saved_enabled"))
...
if enabled:
    if not apps_refresh_ok:
        ...
        if not bool(apps_doc.get("metadata_enrichment_ok", True)):
            warnings.append("Android app metadata enrichment is degraded; labels and scope may be incomplete.")
...
if performance_expected and not performance_doc.get("active"):
    warnings.append("Performance mode is saved for this configured run but the runtime performance controller is inactive.")
...
payload = {
    ...
    "saved_config": {
        ...
        "exit_performance": exit_performance_text == "1",
        "exit_performance_ignore_country_count": int(exit_performance_ignore_count_text or 0),
        "exit_performance_ignored_countries": [x for x in str(saved_perf_ignore_codes or "").split(",") if x],
        "exit_performance_ignored_resolved": str(saved_perf_ignore_resolved or ""),
...
print(
    f"{'Saved Config':<15} "
    f"bypassed-apps={saved.get('bypass_package_count', 0)} "
    f"exit={saved.get('exit_country_count', 0)} "
    f"perf-ignore={saved.get('exit_performance_ignore_country_count', 0)} "
    f"performance={'on' if saved.get('exit_performance') else 'off'} "
    f"strict={'on' if saved.get('exit_strict') else 'off'}"
)
if data.get("enabled"):
    if performance.get("active") and run_mode == "configured":
        print(f"{'Config Apply':<15} saved Apps Tunneling + performance active")
    elif performance.get("active") and run_mode == "configured-exit":
        print(f"{'Config Apply':<15} performance active; Apps Tunneling ignored")
    elif run_mode == "configured":
        print(f"{'Config Apply':<15} saved Apps Tunneling + exit active")
    elif run_mode == "configured-apps":
        print(f"{'Config Apply':<15} saved Apps Tunneling active; exit ignored")
    elif run_mode == "configured-exit":
        print(f"{'Config Apply':<15} saved exit active; Apps Tunneling ignored")
...
if performance.get("active"):
    print(f"{'Performance':<15} active")
    relay_name = " ".join(
        part for part in [
            performance.get("current_exit_fingerprint") or "",
            performance.get("current_exit_nickname") or "",
        ] if part
    ).strip()
    if relay_name:
        print(f"{'Perf Relay':<15} {relay_name}")
    score_parts = []
    current_score = performance.get("current_score", {}) if isinstance(performance.get("current_score"), dict) else {}
    if current_score.get("speed_mbps") not in (None, ""):
        score_parts.append(f"{current_score.get('speed_mbps')} Mbps")
    if current_score.get("latency_ms") not in (None, ""):
        score_parts.append(f"{current_score.get('latency_ms')} ms")
    if score_parts:
        print(f"{'Perf Score':<15} {' | '.join(score_parts)}")
    monitor_state = "degraded" if performance.get("degraded") else "running"
    controller_pid = performance.get("controller_pid")
    if controller_pid not in (None, ""):
        monitor_state = f"{monitor_state} pid={controller_pid}"
    print(f"{'Perf Monitor':<15} {monitor_state}")
if saved.get("exit_performance_ignored_resolved"):
    print(f"{'Perf Ignore':<15} {saved.get('exit_performance_ignored_resolved')}")
...
if active_exit.get("available") and public_exit.get("available"):
    ...
    if active_ip and public_ip and active_ip != public_ip:

...
chroot_cmd_tor() {
  local distro="${1:-}"
  local action json tail_lines
  local yes=0
  json=0
  tail_lines=120

  [[ -n "$distro" ]] || chroot_die "usage: bash path/to/chroot <distro> tor [status|on|start|off|stop|restart|freeze|logs|newnym|doctor|apps|exit|remove|rm] [args...]"
  shift || true
  ...
  case "$action" in
    ...
    exit)
      ...
      case "$exit_action" in
        ...
        set)
          ...
          [[ -n "$value_a" ]] || chroot_die "tor exit set requires a country list or 'strict'"
        ...
        apply)
          ...
          ;;
        *)
          chroot_die "unknown tor exit action: $exit_action (expected: list|refresh|set|apply)"
          ;;
      esac
      ;;
    newnym)
    ...
    restart)
      ...
      ;;
    *)
      chroot_die "unknown tor action: $action (expected: status|on|start|off|stop|restart|freeze|logs|newnym|doctor|apps|exit|remove|rm)"
      ;;
  esac
}

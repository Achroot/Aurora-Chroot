...
chroot_tor_doctor_json() {
  ...
  local nat_probe=0 filter_probe=0 filter6_probe=0 policy4_probe=0 policy6_probe=0
  local nat_error="" filter_error="" filter6_error="" policy4_error="" policy6_error=""
  local active_distro active_at daemon_pid=""
  ...
  # When Tor is already active for this distro, prefer the live routing state over
  # the synthetic probe result. Some host setups report false negatives for the
  # temporary probe chains even though the real Aurora chains are active.
  IFS='|' read -r active_distro active_at <<<"$(chroot_tor_global_active_tsv)"
  daemon_pid="$(chroot_tor_current_pid "$distro" 2>/dev/null || true)"
  if [[ -n "$daemon_pid" && -n "$active_distro" && "$active_distro" == "$distro" ]]; then
    ...
    if [[ -n "$CHROOT_TOR_IPTABLES_BIN" ]] && chroot_tor_iptables_has_rule "$CHROOT_TOR_IPTABLES_BIN" -t filter -C OUTPUT -j "$CHROOT_TOR_CHAIN_FILTER"; then
      filter_probe=1
      filter_error=""
    elif chroot_tor_policy_rules_active_v4 "$distro"; then
      policy4_probe=1
      policy4_error=""
    fi
    if [[ -n "$CHROOT_TOR_IP6TABLES_BIN" ]] && chroot_tor_iptables_has_rule "$CHROOT_TOR_IP6TABLES_BIN" -t filter -C OUTPUT -j "$CHROOT_TOR_CHAIN_FILTER6"; then
      filter6_probe=1
      filter6_error=""
    elif chroot_tor_policy_rules_active_v6 "$distro"; then
      policy6_probe=1
      policy6_error=""
    fi
  fi

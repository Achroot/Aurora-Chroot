...
try:
    ...
    # Some environments cannot parse every compression format via Python stdlib.
    # Caller will run a conservative fallback check via tar CLI.
    sys.exit(10)
...
PY
  ...
  # Fallback validation: path safety via tar list plus conservative link checks.

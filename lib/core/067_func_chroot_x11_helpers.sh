...
chroot_x11_socket_has_listener() {
  ...
  # Check /proc/net/unix first because it reflects the live socket table.
  ...
  # Fall back to ss when /proc/net/unix is hidden or restricted.
  ...
  # If socket checks fail, use the X11 process as the last heuristic.
  chroot_x11_has_running_process
}
...
chroot_x11_enable_display0() {
  ...
  # Leave an already healthy display alone instead of restarting it.
  if chroot_x11_is_display_ready; then
    return 0
  fi

  # Try a clean start first so you do not disrupt a recoverable state.
  if chroot_x11_start_display0 "$timeout_sec"; then
    return 0
  fi

  # If stale process or socket state still blocks startup, escalate to a restart.

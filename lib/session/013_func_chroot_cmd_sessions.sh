chroot_cmd_sessions() {
  local distro="${1:-}"
  [[ -n "$distro" ]] || chroot_die "usage: bash path/to/chroot <distro> sessions [list|status|kill|kill-all] [args]"
  shift || true
  ...
  case "$action" in
    list|status)
      ...
      if [[ "${1:-}" == "--json" ]]; then
        ...
        local count=0 sid pid mode started state cmd device_tz
        device_tz="$(chroot_device_timezone_name 2>/dev/null || printf 'UTC\n')"
        printf 'Sessions for %s\n' "$distro"
        printf 'Local time: %s\n\n' "$device_tz"
        printf '%-28s %-8s %-18s %-8s %-25s %s\n' "Session ID" "PID" "State" "Mode" "Started" "Command"
        printf '%-28s %-8s %-18s %-8s %-25s %s\n' "----------" "---" "-----" "----" "-------" "-------"
        while IFS=$'\t' read -r sid pid mode started state cmd; do
          ...
          [[ -n "$state" ]] || state="-"
          [[ -n "$cmd" ]] || cmd="-"
          printf '%-28s %-8s %-18s %-8s %-25s %s\n' "$sid" "$pid" "$state" "$mode" "$started" "$cmd"
          count=$((count + 1))
        done < <(chroot_session_list_details_tsv "$distro")
        if (( count == 0 )); then
          chroot_info "No tracked sessions for $distro."
        fi
        # Keep SSH connection hints in the sessions view so active services stay actionable.
    ...
    kill)
      ...
      if [[ "$session_id" == "$(chroot_service_desktop_session_id)" ]] && chroot_service_desktop_session_is_tracked "$distro" "$session_id"; then
        chroot_log_run_internal_command service service.stop "$distro" "$distro" service stop "$CHROOT_SERVICE_DESKTOP_SERVICE_NAME" -- chroot_service_desktop_stop "$distro"
      ...
      if declare -F chroot_tor_session_id >/dev/null 2>&1; then
        if [[ "$session_id" == "$(chroot_tor_session_id)" ]]; then
          chroot_log_run_internal_command tor tor.off "$distro" "$distro" tor off -- chroot_tor_locked_teardown_for_distro "$distro"
    ...
    kill-all)
      ...
      if declare -F chroot_tor_locked_teardown_for_distro >/dev/null 2>&1; then
        chroot_log_run_internal_command tor tor.off "$distro" "$distro" tor off -- chroot_tor_locked_teardown_for_distro "$distro" >/dev/null 2>&1 || true
      fi
      ...
      ;;
    *)
      chroot_die "unknown sessions action: $action (expected: list|status|kill|kill-all)"
      ;;
  esac
}

    ...
    def preview_max_hscroll(self):
        height, width = self.stdscr.getmaxyx()
        if self.screen_too_small(height, width):
            return 0
        _, _content_height, _, _, right_width = self.form_panel_layout(height, width)
        rows = self.preview_panel_lines(right_width)
        return self.text_panel_max_hscroll(rows, max(1, right_width - 4))
    ...
    def scroll_preview_horizontal(self, delta):
        max_hscroll = self.preview_max_hscroll()
        self.preview_hscroll = max(
            0,
            min(max_hscroll, self.preview_hscroll + (delta * self.hscroll_step())),
        )
    ...
    def draw_preview_panel(self, top, right_left, right_width, content_height):
        ...
        self.preview_hscroll = max(
            0,
            min(self.preview_hscroll, self.text_panel_max_hscroll(rows, max(1, right_width - 4))),
        )
        ...
        for idx in range(visible_height):
            ...
            if self.preview_hscroll > 0:
                text = text[self.preview_hscroll:]

    ...
    def service_action_summary_lines(self, wrap_width):
        ...
        if action == "install":
            ...
            if selected_builtin == "desktop":
                ...
                lines.append("Normal mode requires an existing paired PC key. Pairing mode prints a short PC browser URL and asks for phone approval.")
        ...
        if action in ("start", "stop", "restart", "remove"):
            ...
            if selected_service == "desktop":
                ...
                if action == "start":
                    ...
                    lines.append("Normal mode needs a previously paired PC key. Pairing modes print a short PC browser URL, then show the command there after approval.")
                elif action == "restart":
    ...
    def tor_action_summary_lines(self, wrap_width):
        ...
        if action == "on":
            ...
            lines.append("Info: Open the full-screen")
            lines.append("Apps Tunneling selector.")
            lines.append("Ticked apps stay on Tor.")
            lines.append("Unticked apps bypass Tor.")
            lines.append("Saved per distro.")
            lines.append("Used only in configured runs.")
            lines.append("Views: all, user,")
            lines.append("system, unknown.")
            lines.append("Open reads the cache first.")
            lines.append("If missing, Aurora rebuilds it.")
        elif action == "exit-tunneling":
            lines.append("Info: Open the full-screen")
            lines.append("Exit Tunneling selector.")
            lines.append("SELECTED is for normal")
            lines.append("country routing.")
            lines.append("IGNORED is for")
            lines.append("performance mode only.")
            lines.append("PERFORMANCE uses live")
            lines.append("relay sampling.")
            lines.append("STRICT only allows")
            lines.append("selected countries.")
            lines.append("Open reads the cache first.")
            lines.append("If missing, Aurora rebuilds it.")
        elif action == "remove":
    ...
    def command_default_summary_lines(self, wrap_width):
        ...
        if self.active_command == "backup":
        ...
        elif self.active_command == "install-local":
            default_path = self.default_install_local_path()
            if default_path:
                lines.append(f"Default install-local path: {default_path}")
                lines.append("The TUI opens a one-path browser that scans this location and lists installable archives underneath it.")
                lines.append("CLI install-local still accepts either a direct archive path or a cache directory.")
            else:
                lines.append("Set a tarball path or cache directory for install-local.")
            ...
            lines.append("Keeps backups, settings, installed distro state, and retained unified Aurora logs.")
        ...
        elif self.active_command == "logs":
            count = str(self.form_values.get("count", "")).strip()
            if count:
                lines.append(f"Shows the latest {count} grouped Aurora command summaries.")
            else:
                lines.append("Shows the latest 10 grouped Aurora command summaries.")
            lines.append("Maximum 50 command groups per view.")
            lines.append("Each result folds Aurora-managed internal steps under the parent command and prints the log directory path at the end.")

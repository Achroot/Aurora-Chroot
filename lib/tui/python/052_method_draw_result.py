    def draw_result(self, height, width):
        ...
        title = "INFO-HUB OUTPUT" if getattr(self, "result_info_mode", False) else "COMMAND OUTPUT"
        draw_box(self.stdscr, content_top, 0, content_height, width - 1, title, self.color(4), self.color(1, curses.A_BOLD))
        ...
        avail = width - 5
        max_hscroll = max(0, max((len(line) for line in self.result_lines), default=0) - max(1, avail))
        self.result_hscroll = max(0, min(self.result_hscroll, max_hscroll))

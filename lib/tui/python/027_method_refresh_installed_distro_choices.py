    def refresh_installed_distro_choices(self, show_error=False):
        ...
        for command in ("service", "sessions", "login", "exec", "mount", "unmount", "confirm-unmount", "backup", "remove"):
            self.set_choice_field_choices(command, "distro", choices, "<no installed distros>")
        if not choices:
            if self.active_command in ("mount", "unmount", "confirm-unmount", "backup", "remove"):

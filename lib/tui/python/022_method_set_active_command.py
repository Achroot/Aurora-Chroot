    def set_active_command(self, command):
        ...
        self.form_index = 0
        if command in ("tor", "service", "sessions", "login", "exec", "mount", "unmount", "backup", "remove"):
        ...
        if command == "install-local":
            ...
            if default_path and not str(self.form_values.get("file", "")).strip():
                self.form_values["file"] = default_path
        if command == "tor":

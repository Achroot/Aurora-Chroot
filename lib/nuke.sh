...
chroot_cmd_nuke() {
  ...
  for distro in "${distros[@]}"; do
    ...
    chroot_log_run_internal_command core unmount "$distro" unmount "$distro" --kill-sessions -- chroot_cmd_unmount "$distro" --kill-sessions || chroot_nuke_fail "nuke blocked: unmount failed for $distro"
  ...
  # Try user-context cleanup first, then fall back to root-context cleanup.

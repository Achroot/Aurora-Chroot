...
chroot_service_start() {
  ...
  # Drop any stale session record before starting the service again.
  ...
  [[ -n "$chroot_backend_bin" ]] || chroot_missing_tool_backend_die chroot
...
pid = os.fork()
if pid > 0:
    # Wait for the first child so the caller only gets the final daemon PID.
...
pid = os.fork()
if pid > 0:
    # Hand the daemon PID back to the caller before this child exits.
    print(pid)
    sys.exit(0)
...
EOF_PY
  ...
  if (( start_rc == 0 )) && [[ -n "$svc_pid" && "$svc_pid" =~ ^[0-9]+$ ]]; then
    ...
    # If the service exits immediately, treat startup as failed instead of "running".

chroot_service_builtin_zsh_script_content() {
  cat <<'EOF'
#!/usr/bin/env bash
# Run this script to refresh the managed root Zsh setup on Arch or Ubuntu.
...
# Pass the distro type when the caller already knows it.
# Otherwise, fall back to package-manager detection.
DISTRO_TYPE="${1:-}"
...
# Install the packages this setup depends on.
case "$DISTRO_TYPE" in
  arch)
...
# Refresh the zsh-autocomplete checkout before wiring it in.
...
# Refresh the zsh-autosuggestions checkout before wiring it in.
...
# Rewrite only the managed /root/.zshrc block so other user content stays intact.
...
# Pull aliases and environment from ~/.bashrc when it exists.
if [[ -f ~/.bashrc ]]; then
  # Provide a small shopt shim so common bashrc defaults do not fail under Zsh.
  shopt() {
    local mode="\${1:-}" opt=""
    shift || true
    case "\$mode" in
...
# Keep the prompt simple and readable on narrow mobile terminals.
PROMPT='%F{cyan}%~%f %F{yellow}%#%f '

# Make completion friendlier for mixed-case names and hidden files.
zstyle ':completion:*' matcher-list 'm:{a-zA-Z}={A-Za-z}' 'r:|[._-]=* r:|=*' 'l:|=* r:|=*'
zstyle ':completion:*' special-dirs true
zstyle ':completion:*' ignore-parents parent pwd
setopt GLOB_DOTS

# Keep autocomplete lists compact so they fit comfortably in Termux.
zstyle ':autocomplete:*' list-lines 5
setopt LIST_PACKED

# Keep menu-navigation keys predictable in mobile terminals.
...
# Keep Right Arrow on forward-char so you can accept autosuggestions at EOL
# without breaking normal cursor movement inside the buffer.
bindkey '^[[C' forward-char
bindkey '^[OC' forward-char
[[ -n \${terminfo[kcuf1]-} ]] && bindkey "\${terminfo[kcuf1]}" forward-char

# Add common editing-key fallbacks for Termux and other mobile terminals.
...
# Let Tab enter menu selection immediately instead of cycling silently.
setopt MENU_COMPLETE
setopt AUTO_PARAM_SLASH
unsetopt AUTO_REMOVE_SLASH

# Keep the fd command name consistent even when the distro ships fdfind.
if command -v fdfind >/dev/null 2>&1 && ! command -v fd >/dev/null 2>&1; then
  alias fd='fdfind'
fi

# Use short aliases for parent-directory jumps.
alias ..='cd ..'
alias ...='cd ../..'
alias ....='cd ../../..'
...
# Patch Bash-only shopt calls so ~/.bashrc can still be sourced from Zsh.
...
# Write a direct Zsh login profile and drop the legacy Bash handoff.
...
# Keep common per-user executables on PATH for Zsh login shells.
...
# Set root's login shell to /usr/bin/zsh.
...
# Verify the managed Zsh setup before reporting success.

...
chroot_service_builtin_resolve() {
  ...
  case "$builtin_id" in
    desktop)
      ...
      ;;
    pcbridge)
      printf 'pcbridge\t/usr/local/sbin/aurora-pcbridge-start\tWindows/WSL file bridge (bootstrap + key-only SFTP + TUI client)\n'
      ;;
    sshd)
...
[
  ...
  },
  {
    ...
    "description": "Windows/WSL file bridge (bootstrap + key-only SFTP + TUI client)"
  },
  {
    "id": "sshd",
    "service_name": "sshd",

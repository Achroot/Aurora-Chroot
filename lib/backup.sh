...
chroot_backup_parse_distro_from_archive_name() {
  ...
  # Match default archive naming: <distro>-<mode>-YYYYMMDD-HHMMSS.tar.*
  if [[ "$stem" =~ ^(.+)-(full|rootfs|state)-[0-9]{8}-[0-9]{6}$ ]]; then
    printf '%s\n' "${BASH_REMATCH[1]}"
    return 0
  fi

  # Fallback for legacy/custom names that still include "-<mode>-".
    ...
    for member in tf.getmembers():
        ...
        if member.issym() or member.islnk():
            ...
            if in_prefix(name, rootfs_prefix):
                ...
                # state/settings.json must never be a link.

...
for distro in installed:
    ...
    for mount_point in mount_points:
        if mount_point == rootfs_base or mount_point.startswith(rootfs_base + '/'):
            rootfs_mounts += 1
  ...
  while IFS= read -r distro; do
    ...
    if [[ -n "$detail_lines" ]]; then
      ...
      while IFS= read -r detail_line; do
        ...
        local detail_row
        printf -v detail_row '  %2d) id=%s pid=%s state=%s mode=%s started=%s cmd=%s\n' \

...
chroot_cmd_settings_show() {
  ...
  "$CHROOT_PYTHON_BIN" - "$snapshot_json" <<'PY'
import json
import sys
import textwrap
...
print(f"{'key':<26} {'current':<12} {'allowed':<22} {'status':<10} description")
print(f"{'-'*26} {'-'*12} {'-'*22} {'-'*10} {'-'*40}")
...
for row in rows:
    ...
    wrapped = textwrap.wrap(desc, width=40) or [""]
    print(f"{key:<26} {current:<12} {allowed:<22} {status:<10} {wrapped[0]}")
    for extra in wrapped[1:]:
        print(f"{'':<26} {'':<12} {'':<22} {'':<10} {extra}")

if any(not row.get("current_valid", True) for row in rows):
    print("\n* current value is outside allowed range/choices")
PY
}

...
chroot_usage_service() {
  printf '%s\n' "bash path/to/chroot <distro> service [list|status|on|start|off|stop|restart|add|install|remove|rm] [args...]"
}

chroot_usage_sessions() {
  printf '%s\n' "bash path/to/chroot <distro> sessions [list|status|kill|kill-all] [args...]"
}

chroot_usage_tor() {
  printf '%s\n' "bash path/to/chroot <distro> tor [status|on|start|off|stop|restart|freeze|logs|newnym|doctor|apps|exit|remove|rm] [args...]"
}

chroot_usage_for_scoped_feature() {
  local feature="${1:-}"
  case "$feature" in
    service) chroot_usage_service ;;
    sessions) chroot_usage_sessions ;;
    tor) chroot_usage_tor ;;
    *) return 1 ;;
  esac
}
  ...
  case "$cmd" in
    service)
      if [[ "$scoped_feature" != "1" ]]; then
        chroot_die "use: $(chroot_usage_service)"
      fi
      ;;
    sessions)
      if [[ "$scoped_feature" != "1" ]]; then
        chroot_die "use: $(chroot_usage_sessions)"
      fi
      ;;
    tor)
      if [[ "$scoped_feature" != "1" ]]; then
        chroot_die "use: $(chroot_usage_tor)"
      fi
      ;;
  esac
  ...
  if [[ "$scoped_feature" != "1" ]]; then
    ...
    if [[ $# -gt 0 ]]; then
      ...
      if [[ -n "$scoped_guess" ]]; then
        chroot_die "unknown command after distro '$cmd': ${1:-}. use: $(chroot_usage_for_scoped_feature "$scoped_guess")"
      fi
    fi
